...
class Schrodinger(object):
    ...
    def __init__(self, x, psi_x0, V_x, k0=None, hbar=1, m=1, t0=0.0):
        ...
        self.x, psi_x0, self.V_x_ = map(np.asarray, (x, psi_x0, V_x))
        N = self.x.size
        assert self.x.shape == (N,)
        assert psi_x0.shape == (N,)
        assert self.V_x_.shape == (N,)
    ...
    def _set_V_x(self, V_x):
        if self.dt_ is not None:
            assert self.V_x_.shape == (self.N,)
            self.V_x_ = V_x
            self._update_operators()
    ...
    def time_step(self, dt, Nsteps=1):
        ...
        # Check that the wave function and its transform are zero close to the boundary
        if not np.allclose(np.abs((self.psi_x[0], self.psi_x[-1]))**2, 0., atol=BOUNDARY_TOL):
            warnings.warn(Warning("Wave function is not {0} at the boundary. The simulation may not be accurate."))

        if not np.allclose(np.abs((self.psi_k[0], self.psi_x[-1]))**2, 0., atol=BOUNDARY_TOL):
            warnings.warn(Warning("Wave function is not {0} at the boundary in the momentum space. The simulation may not be accurate."))
